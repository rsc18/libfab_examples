...
static int initilize(void) {
    ...
    int ret;
    fi_getinfo(FI_VERSION(1, 22), dst_addr, NULL, 0, hints, &info);
    ...
    if (!dst_addr) {
        ...
        printf("Server CXI address: fi_addr_cxi://0x%016" PRIx64 "\n", *addr64);
        fflush(stdout);
    }

    if (dst_addr) {
        uint64_t server_addr = strtoull(dst_addr, NULL, 0);
        printf("Client using server address: 0x%016" PRIx64 "\n", server_addr);
        fi_av_insert(av, &server_addr, 1, &fi_dest, 0, NULL);
    }
    return 0;
}
